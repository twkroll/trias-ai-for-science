...
## Was gegenüber C06-R geändert wird

C06-R ließ noch offen, ob ein daten-/lernbasierter Fall eine hinreichend eigenständige Integrationsleistung der Trias zeigen könnte. Seitdem liegen zwei zusätzliche Tests vor:

1. ML v0.1: `INCONCLUSIVE_LEARNER_ERROR`;
2. inverser Lorenz/SINDy Full Run: `INFORMATIVE_NEGATIVE`.

Der anschließende Comparator-Audit zeigt zudem starke Abdeckung durch System Identification, identifiability/structural error, V&V/Credibility, Workflow/Data Provenance und Claims–Arguments–Evidence/Assurance Cases.

Daher wird nicht mehr empfohlen, eine starke methodologische Eigenständigkeit aus einer angeblich einzigartigen Diagnosefähigkeit abzuleiten.

## Positive Restfunktion

Die Trias kann weiterhin nützlich sein als gemeinsames Schema für sehr verschiedene Fälle:

```text
Sundman:
T analytisch verfügbar -> C praktisch problematisch

Figure-eight:
gleiches T -> verschiedene C_forward-Profile

Equation Discovery:
R -> D -> C_pre -> C_infer -> T_hat

ML:
T/R -> C_sim -> D -> C_ML -> R_hat
```

Die behauptete Stärke liegt in der gemeinsamen Darstellung, nicht in neuen mathematischen Kriterien.

## Non-Claims

C06-R2 behauptet nicht:

- neue V&V-Kategorien;
- neue Provenance-Kategorien;
- neue Identifiability-Theorie;
- neue Claim-Evidence-Argumentation;
- dass Forward-/Inverse-Graphen als solche neu sind;
- dass der inverse negative Run die Trias positiv validiert;
- dass die konzeptionelle Synthese bereits empirisch als nützlicher als Alternativen erwiesen ist.

## Falsifikations-/Abschwächungskriterium
